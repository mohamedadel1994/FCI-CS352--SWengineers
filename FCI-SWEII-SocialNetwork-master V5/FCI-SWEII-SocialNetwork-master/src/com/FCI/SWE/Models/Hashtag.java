...
/**
 * add Rest Service, this service will be called to search for hashtage
 * @param text  provided to add post 
 * @return posts
 */
public class Hashtag {
	...
	public String sort(){
		...
		PreparedQuery pqs = datastore.prepare(gaeQuery);
		for (Entity entity : pqs.asIterable()){
			tempCount.add(Integer.parseInt(entity.getProperty("count").toString()));	
		}
		ArrayList<Integer>hashtagCount =new ArrayList<Integer>(tempCount);
		Collections.sort(hashtagCount );

		for(int i=hashtagCount .size()-1;i>=0;i--){
			for (Entity entity : pqs.asIterable()){
				if (hashtagCount .get(i)==Integer.parseInt(entity.getProperty("count").toString())){
